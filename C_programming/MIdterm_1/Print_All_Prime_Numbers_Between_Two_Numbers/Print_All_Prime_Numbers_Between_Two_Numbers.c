/*
 * Print_All_Prime_Numbers_Between_Two_Numbers.c
 *
...
int main (void)
{
	...
	for(i=first_number;i<second_number;i++)// now i is representing the numbers between two intervals
	{
...
int check_prime(int num) // definition of the function check if the number is prime or not
{
	...
	int i; // counter for loop
	for(i=2;i<num; i++) // check if the number is divided by any number smaller than it (without 1 and 0)
